...
for modelnum in 3; do
	...
	for type in "act" "nppi-dmn" "ppi_seed-VS_thr5"; do
		...
		if [ ${task} == "social+doors" ]; then
			fslmaths ${MAINDIR}/L3_model-${modelnum}_task-socialdoors_n46_flame1+2/L3_task-socialdoors_type-${type}_cnum-4_cname-win-loss_flame1+2.gfeat/cope1.feat/filtered_func_data.nii.gz -add ${MAINDIR}/L3_model-${modelnum}_task-doors_n46_flame1+2/L3_task-doors_type-${type}_cnum-4_cname-win-loss_flame1+2.gfeat/cope1.feat/filtered_func_data.nii.gz filtered_func_sum.nii.gz
			fslmaths filtered_func_sum.nii.gz -div 2 ${MAINDIR}/L3_model-${modelnum}_task-${task}_n46_flame1+2_randomise/L3_model-${modelnum}_task-${task}_type-${type}_cnum-4_cname-win-loss_randomise/type-${type}_model-${modelnum}_randomise_filteredfunc_diff.nii.gz
			rm filtered_func_sum.nii.gz
			printf "Done model $modelnum $type\n"
		fi
		...
		if [ ${task} != "social+doors" ]; then
			#if [${robust} == "no"]; then
			fslmaths ${MAINDIR}/L3_model-${modelnum}_task-social+doors_n46_flame1+2/L3_task-social+doors_type-${type}_cnum-4_cname-win-loss_flame1+2.gfeat/cope1.feat/filtered_func_data.nii.gz	-sub ${MAINDIR}/L3_model-${modelnum}_task-doors_n46_flame1+2/L3_task-doors_type-${type}_cnum-4_cname-win-loss_flame1+2.gfeat/cope1.feat/filtered_func_data.nii.gz type-${type}_model-${modelnum}_randomise_filteredfunc_diff;
